...
async function fetchLeaderboard() {
  loading.value = true
  const { data, error } = await supabase.from(SUPABASE_TABLE).select().order('score', { ascending: false }).limit(100)
...
<template>
  <div class="fixed inset-0 flex items-center justify-center z-[400] bg-black/60">
    <!-- 移动端适配：宽度自适应，最大宽度限制，内边距缩小 -->
    <div
      class="bg-white/95 rounded-lg p-4 sm:p-6 w-full max-w-xs sm:w-80 relative max-h-[80vh] overflow-auto"
    >
