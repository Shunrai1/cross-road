...
export default class User {
  constructor() {
    ...
    // - X 和 Z 坐标（水平向前移动）
    // - Y 坐标（跳起来再落下去）
    // 如果这两个动画的时间、缓动曲线（Ease）不一样，或者你想在小鸡被车撞飞时，让它在 Y 轴上做复杂的翻滚，同时在 X/Z 轴上继续保持原来的惯性…… 结果就是：数学计算会极其复杂，动画极容易冲突甚至崩溃。 （比如水平移动的插值覆盖了垂直跳跃的计算）。
  ...
  listenTouch() {
    ...
    window.addEventListener('touchstart', (e) => {
      if (e.touches.length === 1) { // 确保是单指触摸，避免多指误触
  ...
  //   实现的是当玩家试图往一个 不合法的位置 （比如撞到树、或者试图跳出地图边界）移动时，触发的 “撞墙弹回”（Yoyo / Bouncing back）动画 。
  ...
  setInvincible(flag, duration = 3000) {
    if (flag) {
      ...
      this._setMaterialEffect(null, 0.3)
